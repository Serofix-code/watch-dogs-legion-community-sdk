...
The same update path maps the live rotation inputs without relying on axis inference: pitch input `+0x234` is multiplied by `fPitchRotationSpeed`, yaw input `+0x238` by `fYawRotationSpeed`, and roll input `+0x23C` by `fRollRotationSpeed`. The contiguous movement input vector begins at `+0x21C` and is scaled by `fCameraMoveSpeed` before the camera basis is applied. The read-only observer exposes these inputs and the three accumulators so a live session can establish positive/negative direction conventions.
